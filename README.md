...
1. `sudo pacman -S android-tools mpv vlc phonon-qt6-vlc corectrl mission-center fastfetch kio-admin prismlauncher gwenview partitionmanager qbittorrent sudo pacman cups cups-pdf system-config-printer avahi nss-mdns hplip gutenprint foomatic-db foomatic-db-ppds foomatic-db-nonfree foomatic-db-nonfree-ppds foomatic-db-engine foomatic-db-gutenprint-ppds splix cups-filters ghostscript gsfonts ipp-usb power-profiles-daemon`
